...
from common.utils import angle_difference
...
class RPMNetEarlyFusion(RPMNet):
    ...
    def __init__(self):
        ...
        self.feat_extractor = FeatExtractionEarlyFusion(
            features=['ppf', 'dxyz', 'xyz'], feature_dim=96,
            radius=0.3, num_neighbors=64)


def get_model():
    model = RPMNetEarlyFusion()
    model.to(torch.device("cuda:0"))
    model.load_state_dict(torch.load("../rpmnet/RPM-Net.pth")["state_dict"])
    return model
