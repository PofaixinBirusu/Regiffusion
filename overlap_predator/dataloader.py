...
def collate_fn_descriptor(list_data, config, neighborhood_limits):
    ...
    for ind, (src_pcd, tgt_pcd, T, raw) in enumerate(list_data):
        batched_points_list.append(src_pcd.numpy())
        batched_points_list.append(tgt_pcd.numpy())
        batched_features_list.append(np.zeros(shape=(src_pcd.shape[0], 1)))
        batched_features_list.append(np.zeros(shape=(tgt_pcd.shape[0], 1)))
        batched_lengths_list.append(src_pcd.shape[0])
        batched_lengths_list.append(tgt_pcd.shape[0])
...
def get_dataloader(dataset, config, batch_size=1, num_workers=4, shuffle=True, neighborhood_limits=None):
